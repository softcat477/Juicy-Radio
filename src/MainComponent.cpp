                        ...
                        _stereo_master()
{
    ...
    _stereo_master.setUpdateCycle((sample_per_frame/_device->getCurrentSampleRate())*1000.0);
    ...
    _stereo_master.connectIn(_wires_f2.back());
    ...
    _wires_f2.push_back(std::make_shared<Wire<float, 2>>(sample_per_frame, max_frame_count));
    _stereo_master.connectOut(_wires_f2.back());
    _stereo_out_wire = _wires_f2.back()->getOutPtr();

    // stereo out
    // _stereo_out_gui = _stereo_out.getStereoOut();
    // addAndMakeVisible(*_stereo_out_gui);
    _gui1 = _stereo_out.getStereoOut();
    _gui2 = _stereo_out1.getStereoOut();
    _stereo_out_gui = _stereo_master.getStereoOut();
    ...
    this->_thread_st_master = std::thread(&StereoOut::start, &_stereo_master);
